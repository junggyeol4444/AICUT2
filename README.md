...
`POST /api/projects/{project_id}/run`은 `resume`, `manifest_path`와 `options`를 받습니다. `options`에는 `preprocess`, `output_directory`, `frame_interval_sec`, `coarse_window_sec`, `precision_ranges`, `stt_executable`, `audio_paths`, `language`을 지정할 수 있습니다. `GET /api/projects/{project_id}/job`으로 단계별 체크포인트를 확인하고 `POST /api/projects/{project_id}/cancel`로 실행 중 작업에 협력적 취소를 요청합니다.
