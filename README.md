...
백엔드는 **Python 3.11 이상**을 지원합니다. 브라우저에서 고른 영상은 파일명만 DB에 기록하지 않고 인증된 `/api/media` 업로드를 거쳐 런타임 전용 저장소에 스트리밍한 후 분석합니다. 최대 업로드 크기는 `AICUT_MAX_MEDIA_BYTES`로 제한할 수 있습니다.

YouTube Content Intelligence는 `/api/youtube/references`에 공개 메타데이터와 검증된 제작 패턴만 저장합니다. 외부 분석기는 `/api/youtube/references/analyze`의 `--input`/`--output` 계약으로 연결하며, 타 채널에서 얻을 수 없는 유지율·CTR 등의 비공개 지표는 레퍼런스 데이터로 받지 않습니다. 통합 기획안 대비 구현·실증 상태와 남은 우선순위는 [`docs/INTEGRATED_PLAN_GAP.ko.md`](docs/INTEGRATED_PLAN_GAP.ko.md)에 구분해 기록했습니다.
