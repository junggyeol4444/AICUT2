...
class DatabaseTest(unittest.TestCase):
    ...
    def test_replanning_preserves_discovery_and_stores_reasons(self):
        project = self.db.create_project({"file_path": "/media/live.mkv"})
        manifest = json.loads((Path(__file__).parent / "fixtures" / "analysis-manifest.json").read_text())
        self.db.import_analysis(project["project_id"], manifest)
        plan = [{
            "episode_id": "episode-replanned", "candidate_ids": ["candidate-operation"],
            "structure": {"strategy": "반응 우선"}, "target_type": "LONG",
            "planned_duration_sec": 15, "timeline": [{
                "source_start_sec": 3827, "source_end_sec": 3842, "scene_role": "결과",
                "pacing_mode": "KEEP", "selection_reason": "사건의 결말",
                "pacing_reason": "반응의 여운 보존", "visual_effect": {"type": "zoom"},
            }],
        }]
        counts = self.db.replace_episode_plans(project["project_id"], plan)
        self.assertEqual(counts, {"episodes": 1, "cuts": 1})
        self.assertEqual(len(self.db.list_events(project["project_id"])), 1)
        self.assertEqual(len(self.db.list_candidates(project["project_id"])), 1)
        effect = self.db.get_timeline("episode-replanned")[0]["visual_effect"]
        self.assertEqual(effect["selection_reason"], "사건의 결말")
        self.assertEqual(effect["pacing_reason"], "반응의 여운 보존")
