...
| `plugin/mlt` (어댑터) | Shotcut · Kdenlive | `plugin/common` + `plugin/mlt` 복사 | 문서 생성·엔진 호출은 테스트됨, **두 편집기에서 열어본 적 없음** |
...
## 7. Shotcut · Kdenlive — `plugin/mlt`

둘 다 같은 엔진(MLT) 위에 있고, 프로젝트 파일이 **MLT XML**이다. 그래서 어댑터
하나가 둘을 다 맡는다 — Shotcut은 이 파일을 바로 열고, Kdenlive는 프로젝트로
가져온다.

```bash
python plugin/mlt/aicut_mlt.py <edit-model>.json
python plugin/mlt/aicut_mlt.py --engine /방송/live.mkv    # 4장의 버튼
```

`--fps`(모델에 없으면 묻는다), `--out`, `--name`, `--mode`.

### MLT라서 다른 것

* **`out`은 마지막 프레임이다.** Resolve와 같고 Premiere와 반대다. 여기서 1이
  어긋나면 모든 컷이 한 프레임씩 길거나 짧아지고, 내보내기 전까지 아무도 모른다.
  그래서 상수로 적어놨다.
* **프레임 레이트가 분수다.** 29.97이 아니라 30000/1001. 소수로 쓰면 천 프레임마다
  한 프레임씩 밀리고, 6시간 방송이면 분 단위가 된다.
* **검은 배경 트랙**을 깔아준다. 두 편집기 다 자기가 저장할 때 그렇게 하고, 없으면
  타임라인이 앉을 자리가 없다.

### 검증 상태

Shotcut도 Kdenlive도 이 환경에 없다. **열어본 적 없다.** 확인한 것:

* XML이 제대로 만들어지는지(파싱되는지)
* 모든 항목이 모델 리더가 "살아남는다"고 한 프레임과 정확히 같은지
* 플레이리스트가 끊김 없이 이어지고, 트랙터가 말하는 길이가 실제 길이와 같은지
  (틀리면 끝에 검은 화면이 남거나 마지막 컷이 잘린다)
