...
> 프로젝트 문서는 PR 충돌이 잦은 루트 `README.md`와 분리해 이 파일에서 관리합니다. PR 전에는 `npm run check:pr`로 미해결 인덱스, 충돌 마커 및 바이너리 diff를 검사하세요. GitHub 기준 브랜치가 갱신된 경우에는 해당 원격 브랜치를 fetch/rebase한 뒤 이 검사를 다시 실행해야 합니다.
