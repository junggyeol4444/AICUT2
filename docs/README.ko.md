...
> 이 후속 작업은 이미 병합된 PR #1 커밋 `e490252` 위에 다시 구성되어 동일 파일의 add/add 충돌을 제거했습니다. 추가 프로젝트 문서는 루트 `README.md`와 분리해 이 파일에서 관리합니다. PR 전에는 `npm run check:pr`로 미해결 인덱스, 충돌 마커 및 바이너리 diff를 검사하세요.
