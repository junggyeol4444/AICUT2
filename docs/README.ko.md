...
멀티모달 신호 계층은 분리된 PCM 트랙에서 RMS·peak·zero-crossing 원시 특징을 측정하고, 외부 비전 모델의 인물·상황·움직임·표정·화면 사건 관찰을 검증합니다. 이 신호를 STT와 1·2차 스캔 시간창에 정렬해 AI가 해석할 분석 패키지로 저장하며, 텐션이나 중요도 임계값은 여기서 하드코딩하지 않습니다.

콘텐츠 발견 실행기는 저장된 멀티모달 시간창을 외부 AI producer에 전달하고 사건·후보·에피소드 매니페스트를 받습니다. 입력에는 화면 상태가 아닌 사건을 경계로 삼고, 0개 후보와 비시간순 구성을 허용하며, 모든 제작 판단에 근거를 요구한다는 계약이 포함됩니다. 결과는 사건 참조, 원본 시간 범위, 독립성 점수, 호흡 모드를 검증한 뒤에만 DB에 반영됩니다.

동적 기획 실행기는 사람 검토 결과가 `MAKE` 또는 `COMBINE`인 후보만 받아 콘텐츠별 구조와 비선형 컷 순서를 생성합니다. 목표 길이는 강제가 아닌 힌트로 전달되며, 모든 컷은 선택 근거와 `KEEP/TRIM/CUT` 호흡 근거를 포함해야 저장됩니다. 기획 결과 교체 시 이미 검토한 사건과 후보는 삭제하지 않습니다.
...
- `backend/signals.py` — PCM 오디오 특징, 비전 관찰 검증 및 시간축 멀티모달 패키징
- `backend/discovery.py` — 외부 AI producer 실행, 사건 기반 후보·비선형 에피소드 결과 검증
- `backend/planning.py` — 검토된 후보 기반 동적 구조·장면 선택·설명 가능한 호흡 계획
