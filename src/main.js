...
function bindCommon() {
  ...
  $$('[data-action="package"]').forEach(el=>el.onclick=async()=>{await withFallback(()=>api.renderEpisode('episode-operation',{execute:false}),null);showModal('package')});
  $$('[data-action="export-json"]').forEach(el=>el.onclick=()=>toast('편집 계획 JSON을 내보냈습니다.'));
  $$('[data-action="publish"]').forEach(el=>el.onclick=async()=>{await withFallback(()=>api.reviewEpisode('episode-01',true),null);state.modal=null;$('#modal-root').innerHTML='';toast('검수 승인 완료 · 비공개 업로드 큐에 등록했습니다.')});
