...
const state = { view: 'workspace', selectedCandidate: '01', filter: '전체', modal: null, toastTimer: null, runtimeOnline: false };
...
function candidateCard(c) {
  return `<article class="candidate ${state.selectedCandidate===c.id?'selected':''}" data-candidate="${c.id}" tabindex="0"><div class="candidate-head"><span class="candidate-no">${c.id}</span><span class="decision" style="--tone:${c.color}"><i></i>${c.decision}</span><span class="score">독립성 <b>${c.score}</b></span></div><h3>${c.title}</h3><p>${c.summary}</p><div class="candidate-meta"><span>◷ ${c.time}</span><span>▱ 장면 ${c.scenes}개</span>${c.tags.map(t=>`<em>${t}</em>`).join('')}</div></article>`;
}
...
function reviewView() {
  const project=PROJECTS[0], c=CANDIDATES.find(x=>x.id===state.selectedCandidate);
  return `<div class="page review-page">${pageHeader('BROADCAST ANALYSIS','콘텐츠 후보 검토','AI가 방송 전체에서 발견한 사건을 검토하고 제작 여부를 결정하세요.',`<div class="source"><span>▶</span><div><b>${project.file}</b><small>${project.duration} · ${project.size} · ${project.tracks} audio tracks</small></div><button>•••</button></div>`)}${pipeline()}
    <section class="workspace"><div class="list-pane panel"><div class="list-header"><div><h2>발견된 후보 <span>04</span></h2><p>화면이 아니라 사건 단위로 분류했습니다</p></div><div class="filters">${['전체','제작','결합 검토','제작 안함'].map(x=>`<button class="${state.filter===x?'active':''}" data-filter="${x}">${x}</button>`).join('')}</div></div><div id="candidate-list">${CANDIDATES.filter(c=>state.filter==='전체'||c.decision===state.filter).map(candidateCard).join('')}</div></div><aside class="candidate-detail panel">${candidateDetail(c)}</aside></section>
  </div>`;
}
...
function bindCommon() {
  ...
  $$('[data-project]').forEach(el=>el.onclick=()=>setView('review'));
  ...
  $$('[data-action="package"]').forEach(el=>el.onclick=async()=>{await withFallback(async()=>{await api.renderEpisode('episode-operation',{execute:false});return api.packageEpisode('episode-operation',{execute:false,metadata:{title_options:EPISODE.titleOptions,description:EPISODE.description,tags:EPISODE.tags,chapters:[]},thumbnail_timestamps:[12,94,252]})},null);showModal('package')});
  $$('[data-action="export-json"]').forEach(el=>el.onclick=()=>toast('편집 계획 JSON을 내보냈습니다.'));
  $$('[data-action="publish"]').forEach(el=>el.onclick=async()=>{await withFallback(async()=>{await api.reviewEpisode('episode-operation',true);return api.publishEpisode('episode-operation','PRIVATE')},null);state.modal=null;$('#modal-root').innerHTML='';toast('검수 승인 완료 · 비공개 업로드 큐에 등록했습니다.')});
...
withFallback(() => api.health(), null).then(health => {
