...
function workspaceView() {
  const active=projects().find(item=>!['COMPLETE','NO_CONTENT','FAILED'].includes(item.status))||projects()[0];
  return `<div class="page">${pageHeader('AUTONOMOUS CONTENT PRODUCER','좋은 방송을, 완성된 콘텐츠로.','긴 생방송에서 사건을 이해하고 독립적인 YouTube 콘텐츠를 발견합니다.',`<button class="accent-button large" data-action="new-project">${icons.plus} 방송 가져오기</button>`)}
    <section class="hero-grid"><article class="active-project"><div class="active-top"><div><span class="pulse"></span><b>${active?'최근 프로젝트':'대기 중'}</b></div><button data-action="open-review">상세 보기 →</button></div><h2>${active?.name||'등록된 프로젝트 없음'}</h2><p>${active?`${active.status} · ${active.file}`:'새 방송을 등록하면 실제 진행 상태가 표시됩니다.'}</p><div class="active-progress"><i style="width:${active?.progress||0}%"></i></div><div class="active-meta"><span><b>${active?.progress||0}%</b> 전체 진행률</span><span><b>${active?.candidates||0}</b> 발견 후보</span><span><b>${active?.episodes||0}</b> 에피소드</span><span><b>${state.uploads.length}</b> 업로드 작업</span></div></article>
    ...
    <section class="dashboard-grid"><article class="panel learning"><div class="panel-title"><div><span class="eyebrow">LEARNING LOOP</span><h3>저장된 실제 학습 데이터</h3></div><span class="spark">✦</span></div><div class="loop-row"><div><b>${projects().length}</b><small>분석 프로젝트</small></div><i>→</i><div><b>${state.sourcePairs.length}</b><small>원본↔완성본</small></div><i>→</i><div><b>${state.uploads.filter(item=>item.status==='COMPLETE').length}</b><small>게시 완료</small></div></div></article>
    <article class="panel metric-summary"><div class="panel-title"><div><span class="eyebrow">CHANNEL QUALITY</span><h3>최근 캘리브레이션</h3></div><button data-view="calibration">열기</button></div><strong>${state.calibrations[0]?.score??'—'}</strong><span>/ 100</span><div class="quality-bar"><i></i></div><small>${state.calibrations.length}개 프로파일</small></article></section>
  </div>`;
}

function projectsView() {
  const rows=projects(), running=rows.filter(p=>!['COMPLETE','NO_CONTENT','FAILED','REVIEW_PENDING'].includes(p.status)).length;
  return `<div class="page">${pageHeader('PROJECT LIBRARY','프로젝트','모든 방송 분석 작업과 산출물을 관리합니다.',`<button class="accent-button" data-action="new-project">${icons.plus} 새 프로젝트</button>`)}
    <div class="toolbar"><div class="search-field">${icons.search}<input placeholder="프로젝트 검색" /></div><div class="segmented"><button class="active">전체 ${rows.length}</button><button>진행 중 ${running}</button><button>검수 대기 ${rows.filter(p=>p.status==='REVIEW_PENDING').length}</button><button>완료 ${rows.filter(p=>p.status==='COMPLETE').length}</button></div><button class="sort">최근 수정순 ⇅</button></div>
...
function pipeline() {
  ...
  const current=Math.max(0,PIPELINE.findIndex(item=>item.key===project?.status));
  const progress=project?.progress||0;
  return `<section class="pipeline panel"><div class="pipeline-row">${PIPELINE.map((s,i)=>`<div class="stage ${i<current?'done':i===current?'current':''}"><i>${i<current?'✓':i+1}</i><span>${s.label}</span></div>${i<PIPELINE.length-1?'<hr>':''}`).join('')}</div><div class="progress-copy"><span><b>${PIPELINE[current]?.label||'대기'}</b> · ${candidates().length}개의 후보</span><span>전체 진행률 <b>${progress}%</b></span></div><div class="progress"><i style="width:${progress}%"></i></div></section>`;
}
...
function calibrationView() {
  if(state.runtimeOnline){const profile=state.calibrations[0];return `<div class="page">${pageHeader('CHANNEL CALIBRATION','캘리브레이션','SQLite에 저장된 실제 캘리브레이션 프로파일입니다.',`<button class="accent-button" data-action="calibrate">새로 측정</button>`)}<section class="calibration-summary panel"><div><h2>${profile?.name||'저장된 프로파일 없음'}</h2><p>${state.calibrations.length}개 프로파일</p></div><div class="overall-score"><b>${profile?.score??'—'}</b><span>/ 100</span></div></section></div>`}
...
function logsView() {
  const rows=state.runtimeOnline?(state.logs||[]).map(item=>[new Date(item.created_at).toLocaleTimeString('ko-KR'),item.stage||item.level||'INFO',item.message]):LOGS;
  return `<div class="page">${pageHeader('PROCESS OBSERVABILITY','처리 로그','멀티모달 분석과 콘텐츠 제작 파이프라인의 실행 기록입니다.',`<div class="live-indicator"><i></i>LIVE</div>`)}<div class="toolbar"><div class="search-field wide">${icons.search}<input placeholder="메시지 또는 상태 검색" /></div><div class="segmented"><button class="active">전체</button><button>정보</button><button>경고</button><button>오류</button></div><button class="secondary-button">로그 내보내기</button></div><section class="log-console"><header><span>TIME</span><span>STAGE</span><span>MESSAGE</span></header>${rows.map((l,i)=>`<div><time>${l[0]}</time><b>${l[1]}</b><p>${l[2]}</p><span class="log-level ${i===2?'warn':''}">${i===2?'WARN':'INFO'}</span></div>`).join('')}<footer><i></i> 새 이벤트를 기다리는 중...</footer></section></div>`;
}
...
function newProjectModal() {
  return `<div class="modal-backdrop"><section class="modal new-project-modal"><header><div><span class="eyebrow">NEW BROADCAST</span><h2>새 방송 분석</h2><p>원본은 로컬에서 처리되며 편집 계획과 분석 결과가 보존됩니다.</p></div><button class="close" data-close>×</button></header><div class="dropzone"><span>＋</span><h3>생방송 파일을 선택하거나 놓으세요</h3><p>MP4, MKV · 최대 크기는 서버 설정을 따릅니다</p><button class="secondary-button">파일 선택</button><input type="file" accept="video/mp4,video/x-matroska"></div><div class="form-grid"><label>파이프라인 options JSON<small>모델 실행기·렌더 옵션</small><textarea id="pipeline-options">{}</textarea></label></div><footer><button class="secondary-button" data-close>취소</button><button class="accent-button" data-action="start-analysis">전체 방송 분석 시작 →</button></footer></section></div>`;
}
...
function bindCommon() {
  ...
  $$('[data-action="start-analysis"]').forEach(el=>el.onclick=async()=>{const input=$('.dropzone input');const file=input?.files?.[0];if(!file){toast('분석할 영상 파일을 선택하세요.');return}let options;try{options=JSON.parse($('#pipeline-options')?.value||'{}')}catch{toast('파이프라인 options JSON을 확인하세요.');return}const media=await withFallback(()=>api.uploadMedia(file),null);if(!media){toast('영상 업로드에 실패했습니다.');return}const project=await withFallback(()=>api.createProject({file_path:media.file_path,name:file.name.replace(/\.[^.]+$/,''),target_duration_hint:'AI'}),null);if(project)await withFallback(()=>api.runProject(project.project_id,{options}),null);state.modal=null;$('#modal-root').innerHTML='';toast('영상을 저장하고 미디어 파싱을 시작했습니다.')});
