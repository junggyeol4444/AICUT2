...
class PipelineManager:
    ...
    def _prune_completed_jobs(self, keep: int = 128) -> None:
        """Bound completed Future retention while preserving recent job status."""
        with self._lock:
            completed = [key for key, future in self._jobs.items() if future.done()]
            for project_id in completed[:-keep] if keep else completed:
        ...
        try:
            ...
            if options.get("render"):
                ...
                for index, episode in enumerate(episodes):
                    ...
                    cues = (options.get("subtitle_cues") or {}).get(episode_id)
                    if cues is not None:
                        style = (options.get("subtitle_style_profiles") or {}).get(episode_id)
                        if not style:
                            raise ValueError(f"{episode_id} 자막 cue에 필요한 subtitle style profile이 없습니다.")
                        cuts = [item for item in self.database.get_timeline(episode_id) if item["pacing_mode"] != "CUT"]
                        output_duration = sum(item["source_end_sec"] - item["source_start_sec"] for item in cuts)
                        subtitle_path = write_ass_subtitles(
                            cues, style, artifact_root / "subtitles" / f"{episode_id}.ass", output_duration,
