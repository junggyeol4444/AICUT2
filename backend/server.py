...
from .signals import assemble_analysis_windows, extract_pcm_features, validate_visual_observations
from .discovery import build_discovery_command, run_discovery
from .planning import build_planning_command, run_planning
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            elif path.startswith("/api/projects/") and path.endswith("/understanding"):
                self.json(DB.list_analysis_windows(path.split("/")[3]))
            else:
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/projects":
                ...
                project = DB.get_project(project_id)
                audio_features = []
                for track_index, audio_path in enumerate(payload.get("audio_paths", [])):
                    audio_features.extend(extract_pcm_features(
                        audio_path, track_index, float(payload["audio_window_sec"]),
                    ))
                visual = validate_visual_observations(payload.get("visual_observations", []), project["duration_sec"])
                windows = assemble_analysis_windows(
                    DB.list_scan_windows(project_id), DB.list_transcript(project_id), audio_features, visual,
                )
                DB.replace_analysis_windows(project_id, windows)
                self.json({"project_id": project_id, "window_count": len(windows), "windows": windows}, HTTPStatus.CREATED)
                ...
                if not isinstance(executable, list):
                    raise ValueError("executable은 셸 문자열이 아닌 인자 배열이어야 합니다.")
                work_directory = payload.get("work_directory") or str(ROOT / "artifacts" / project_id / "discovery")
                input_path = str(Path(work_directory) / "understanding-input.json")
                output_path = str(Path(work_directory) / "discovery-output.json")
                if payload.get("execute", False):
                    result = run_discovery(
                        executable, DB.list_analysis_windows(project_id), project["duration_sec"], work_directory,
                    )
                    counts = DB.import_analysis(project_id, result["manifest"])
                    self.json({"project_id": project_id, **counts, "command": result["command"]}, HTTPStatus.CREATED)
                else:
                    self.json({
                        "project_id": project_id, "dry_run": True,
                        "command": build_discovery_command(executable, input_path, output_path),
                    })
            elif path.startswith("/api/projects/") and path.endswith("/plan"):
                project_id = path.split("/")[3]
                project = DB.get_project(project_id)
                executable = payload.get("executable")
                if not isinstance(executable, list):
                    raise ValueError("executable은 셸 문자열이 아닌 인자 배열이어야 합니다.")
                work_directory = payload.get("work_directory") or str(ROOT / "artifacts" / project_id / "planning")
                input_path = str(Path(work_directory) / "planning-input.json")
                output_path = str(Path(work_directory) / "edit-plan.json")
                if payload.get("execute", False):
                    result = run_planning(
                        executable, DB.list_candidates(project_id), DB.list_events(project_id),
                        DB.list_analysis_windows(project_id), project["duration_sec"], work_directory,
                        project.get("target_duration_hint"),
                    )
                    counts = DB.replace_episode_plans(project_id, result["plan"]["episodes"])
                    self.json({"project_id": project_id, **counts, "command": result["command"]}, HTTPStatus.CREATED)
                else:
                    self.json({
                        "project_id": project_id, "dry_run": True,
                        "command": build_planning_command(executable, input_path, output_path),
                    })
