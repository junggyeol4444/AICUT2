...
class Database:
    ...
    def list_events(self, project_id: str) -> list[dict[str, Any]]:
        with self.connect() as connection:
            rows = connection.execute(
                "SELECT * FROM events WHERE project_id=? ORDER BY rowid", (project_id,),
            ).fetchall()
            result = []
            for row in rows:
                item = self._decode(self._decode(dict(row), "people_json"), "relations_json")
                mentions = connection.execute(
                    "SELECT source_start_sec start_sec,source_end_sec end_sec,role "
                    "FROM event_mentions WHERE event_id=? ORDER BY source_start_sec", (item["event_id"],),
                ).fetchall()
                item["mentions"] = [dict(mention) for mention in mentions]
                result.append(item)
        return result

    def replace_episode_plans(self, project_id: str, episodes: list[dict[str, Any]]) -> dict[str, int]:
        """Replace only planning output, preserving reviewed discovery events and candidates."""
        with self.connect() as connection:
            if not connection.execute("SELECT 1 FROM projects WHERE project_id=?", (project_id,)).fetchone():
                raise KeyError(project_id)
            connection.execute("DELETE FROM episodes WHERE project_id=?", (project_id,))
            ...
            for episode in episodes:
                connection.execute(
                    ...
                    (episode_id,project_id,candidate_ids_json,planned_structure_json,target_type,
                     planned_duration_sec,render_status,review_status,metadata_json)
                    VALUES(?,?,?,?,?,?,?,?,?)""",
                    (episode["episode_id"], project_id, json.dumps(episode["candidate_ids"]),
                     json.dumps(episode["structure"], ensure_ascii=False), episode["target_type"],
                     episode["planned_duration_sec"], "PENDING", "PENDING",
                     json.dumps(episode.get("metadata", {}), ensure_ascii=False)),
                )
                for order, cut in enumerate(episode["timeline"], 1):
                    effect = dict(cut.get("visual_effect", {}))
                    effect["selection_reason"] = cut["selection_reason"]
                    effect["pacing_reason"] = cut["pacing_reason"]
                    connection.execute(
                        """INSERT INTO edit_timeline
                        (episode_id,sequence_order,source_start_sec,source_end_sec,speaker_tag,scene_role,
                         pacing_mode,visual_effect_json,subtitle_ref) VALUES(?,?,?,?,?,?,?,?,?)""",
                        (episode["episode_id"], order, cut["source_start_sec"], cut["source_end_sec"],
                         cut.get("speaker_tag", "UNKNOWN"), cut["scene_role"], cut["pacing_mode"],
                         json.dumps(effect, ensure_ascii=False), cut.get("subtitle_ref")),
                    )
                    cut_count += 1
            status = "PLANNING" if episodes else "NO_CONTENT"
            connection.execute(
                "UPDATE projects SET status=?,progress=?,updated_at=? WHERE project_id=?",
                (status, 72 if episodes else 100, now(), project_id),
            )
            self._log(connection, project_id, status, f"동적 편집 계획 저장 완료 · 에피소드 {len(episodes)}, 컷 {cut_count}")
        return {"episodes": len(episodes), "cuts": cut_count}
    ...
    def list_scan_windows(self, project_id: str) -> list[dict[str, Any]]:
        with self.connect() as connection:
            rows = connection.execute(
                "SELECT * FROM scan_windows WHERE project_id=? ORDER BY pass_kind,start_sec", (project_id,),
            ).fetchall()
        return [dict(row) for row in rows]
    ...
    def list_transcript(self, project_id: str) -> list[dict[str, Any]]:
        with self.connect() as connection:
            rows = connection.execute(
                "SELECT * FROM transcript_segments WHERE project_id=? ORDER BY start_sec,track_index", (project_id,),
            ).fetchall()
        result = []
        for row in rows:
            value = dict(row)
            value["words"] = json.loads(value.pop("words_json"))
            result.append(value)
        return result

    def replace_analysis_windows(self, project_id: str, windows: list[dict[str, Any]]) -> int:
        with self.connect() as connection:
            if not connection.execute("SELECT 1 FROM projects WHERE project_id=?", (project_id,)).fetchone():
                raise KeyError(project_id)
            connection.execute("DELETE FROM analysis_windows WHERE project_id=?", (project_id,))
            connection.executemany(
                """INSERT INTO analysis_windows(project_id,pass_kind,start_sec,end_sec,payload_json,created_at)
                VALUES(?,?,?,?,?,?)""",
                [(project_id, item["pass_kind"], item["start_sec"], item["end_sec"],
                  json.dumps(item, ensure_ascii=False), now()) for item in windows],
            )
        return len(windows)

    def list_analysis_windows(self, project_id: str) -> list[dict[str, Any]]:
        with self.connect() as connection:
            rows = connection.execute(
                "SELECT * FROM analysis_windows WHERE project_id=? ORDER BY pass_kind,start_sec", (project_id,),
            ).fetchall()
        result = []
        for row in rows:
            value = dict(row)
