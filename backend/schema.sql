...
CREATE TABLE IF NOT EXISTS analysis_windows (
  analysis_window_id INTEGER PRIMARY KEY AUTOINCREMENT,
  project_id TEXT NOT NULL REFERENCES projects(project_id) ON DELETE CASCADE,
  pass_kind TEXT NOT NULL CHECK (pass_kind IN ('COARSE','PRECISION')),
  start_sec REAL NOT NULL,
  end_sec REAL NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_analysis_windows_project ON analysis_windows(project_id, pass_kind, start_sec);
