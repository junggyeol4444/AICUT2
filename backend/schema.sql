...
CREATE TABLE IF NOT EXISTS youtube_references (
  ...
  media_discarded INTEGER NOT NULL DEFAULT 0 CHECK(media_discarded IN (0,1)),
  analyzed_at TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS production_patterns (
  pattern_id TEXT PRIMARY KEY,
  reference_id TEXT NOT NULL REFERENCES youtube_references(reference_id) ON DELETE CASCADE,
  kind TEXT NOT NULL,
  title TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_youtube_references_channel ON youtube_references(channel_ref, analyzed_at DESC);
CREATE INDEX IF NOT EXISTS idx_production_patterns_kind ON production_patterns(kind, confidence DESC);
