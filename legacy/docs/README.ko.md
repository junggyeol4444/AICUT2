...
로컬 작업 큐는 `ffprobe → 전처리 → 스캔 계획 → WhisperX → AI producer → 분석 결과 반영`을 체크포인트 기반 작업으로 연결합니다. Producer 실행기는 `--input <분석 패키지 JSON> --output <결과 JSON>` 계약을 사용하며, 런타임은 ID 참조·원본 시간 범위·판단값·호흡 모드를 검증한 사건·언급·후보·에피소드·컷만 단일 트랜잭션으로 반영합니다. 분석기가 연결되지 않은 경우에는 앞 단계를 완료한 뒤 `UNDERSTANDING` 상태에서 명시적으로 대기합니다.
